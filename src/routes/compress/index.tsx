    ...
    <div class="min-h-screen bg-gradient-to-br from-orange-50 via-white to-yellow-50">
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {SLUGS.length === 0 && (
            <div class="col-span-full text-gray-500">Compression tools are coming soon.</div>
          )}
          {SLUGS.map((slug) => (
            <a key={slug} href={`${langPrefix}/compress/${slug}`} class="block rounded-lg border border-gray-200 bg-white p-4 hover:shadow-md transition-shadow group">
              <div class="text-base font-semibold text-gray-800 group-hover:text-orange-600">{humanize(slug)}</div>
