...
import { FileText, Archive, FileSpreadsheet, ArrowLeft, Search, Download, Box, Music, Image, Smartphone, Download as DownloadIcon, Package, CheckCircle } from 'lucide-react';
...
export const SamplesHub: React.FC = () => {
  ...
  const categoryTitleMap: Record<string, string> = {
    "DOCUMENT SAMPLES": "samples_page.categories.document",
    "DATA SAMPLES": "samples_page.categories.data",
    "APPLICATION SAMPLES": "samples_page.categories.application",
    "3D & DESIGN SAMPLES": "samples_page.categories.design_3d",
    "IMAGE & VIDEO SAMPLES": "samples_page.categories.image_video",
    "AUDIO SAMPLES": "samples_page.categories.audio",
    "ARCHIVE SAMPLES": "samples_page.categories.archive"
  };

  const sampleCategories = [
    {
      title: "DOCUMENT SAMPLES",
      titleKey: "samples_page.categories.document",
    ...
    {
      title: "DATA SAMPLES",
      titleKey: "samples_page.categories.data",
    ...
    },
    {
      ...
      titleKey: "samples_page.categories.application",
    ...
    },
    {
      ...
      titleKey: "samples_page.categories.design_3d",
    ...
    },
    {
      ...
      titleKey: "samples_page.categories.image_video",
    ...
    {
      title: "AUDIO SAMPLES",
      titleKey: "samples_page.categories.audio",
    ...
    },
    {
      ...
      titleKey: "samples_page.categories.archive",
  ...
  const heroTitle = t('samples_page.hero.title');
  const heroDescription = t('samples_page.hero.description');
  const backLabel = t('samples_page.back_button');
  const searchPlaceholder = t('samples_page.search.placeholder');
  const exampleOne = t('samples_page.search.example_one');
  const exampleTwo = t('samples_page.search.example_two');
  const searchResultsLabel = t('samples_page.search.results', { count: filteredCategories.reduce((sum, cat) => sum + cat.formats.length, 0) });
  const downloadLabel = t('samples_page.actions.download');
  const noResultsTitle = t('samples_page.no_results.title');
  const noResultsDescription = t('samples_page.no_results.description', { exampleOne, exampleTwo });
  const noResultsClear = t('samples_page.no_results.clear');
  const featureTitle = t('samples_page.features.title');
  const featureItems = t('samples_page.features.items', { returnObjects: true }) as Array<{ title: string; description: string }>;
  const featureIconConfig = [
    { bg: 'bg-blue-100', icon: <DownloadIcon className="w-6 h-6 text-blue-600" /> },
    { bg: 'bg-purple-100', icon: <Package className="w-6 h-6 text-purple-600" /> },
    { bg: 'bg-green-100', icon: <CheckCircle className="w-6 h-6 text-green-600" /> }
  ];
