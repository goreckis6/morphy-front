    ...
    <header class="bg-white shadow-lg sticky top-0 z-40">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="hidden md:flex items-center space-x-6">
            <nav class="flex items-center space-x-6">
              <a href={getNavHref('/converters')} class="flex items-center space-x-2 text-gray-700 hover:text-blue-600 transition-colors group">
                <ArrowLeftRight class="w-5 h-5 text-blue-600 group-hover:text-blue-700" />
          ...
          >
            {showMobileMenu.value ? <X class="w-6 h-6" /> : <Menu class="w-6 h-6" />}
          </button>
        </div>
