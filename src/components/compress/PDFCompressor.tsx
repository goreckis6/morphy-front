...
export const PDFCompressor: React.FC = () => {
  ...
  const handleBatchFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const pdfFiles = files.filter((file: File) => 
      file.type === 'application/pdf' || file.name.toLowerCase().match(/\.(pdf)$/)
    );
    ...
    if (pdfFiles.length !== files.length) {
      setError('Some files were not PDF files and were skipped.');
    }
  };
  ...
  const handleBatchDownloadZip = async () => {
    ...
    try {
      ...
      for (const result of batchResults) {
        const nameWithoutExt = result.file.name.replace(/\.(pdf)$/i, '');
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `compressed_pdfs_${new Date().getTime()}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  return (
    <>
      <Helmet>
        <title>{t('compress_pdf.meta_title')}</title>
        <meta name="description" content={t('compress_pdf.meta_description')} />
        <meta name="keywords" content={t('compress_pdf.meta_keywords')} />
        <link rel="canonical" href="https://morphyhub.com/compress/pdf" />
      </Helmet>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-400 transition-colors">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {batchMode ? t('compress_pdf.upload_batch') : t('compress_pdf.upload_single')}
                </h3>
                <p className="text-gray-600 mb-4">
                  {batchMode 
                    ? t('compress_pdf.upload_text_batch')
                    : t('compress_pdf.upload_text_single')
                  }
                </p>
                <p className="text-sm text-gray-500 mb-4">
                  {batchMode 
                    ? t('compress_pdf.file_limits_batch')
                    : t('compress_pdf.file_limits_single')
                  }
                </p>
                <input
                  ref={fileInputRef}
              ...
              {previewUrl && !batchMode && selectedFile && (
                <div className="mt-6">
                  <h4 className="text-lg font-semibold mb-4">{t('compress_pdf.file_preview')}</h4>
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="mt-3 text-center">
                      ...
                      <p className="text-blue-600 font-medium mt-2">
                        {t('compress_pdf.will_compress_with')}: {quality}% {t('compress_pdf.quality')}
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    return (
                      <>
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="text-lg font-semibold">{t('compress_pdf.selected_files')} ({batchFiles.length})</h4>
                          ...
                          <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                            <div className="flex items-center">
                              ...
                              <span className="text-sm text-orange-700">
                                {t('compress_pdf.batch_size_warning')}
                              </span>
                            </div>
                          </div>
                        )}
              ...
              {compressionStats && !batchMode && (
                <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-gray-600">{t('compress_pdf.original_size')}: <strong className="text-gray-900">{formatFileSize(compressionStats.originalSize)}</strong></p>
                      <p className="text-sm text-gray-600">{t('compress_pdf.compressed_size')}: <strong className="text-green-600">{formatFileSize(compressionStats.newSize)}</strong></p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold text-green-600">{compressionStats.savings.toFixed(1)}%</p>
                      <p className="text-xs text-gray-500">{t('compress_pdf.savings')}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                >
                  {isCompressing ? (
                    <div className="flex items-center justify-center">
                      ...
                      {t('compress_pdf.compressing')}
                    </div>
                  ) : (
                    <div className="flex items-center justify-center">
                      <Minimize2 className="w-5 h-5 mr-2" />
                      {batchMode ? t('compress_pdf.compress_files', { count: batchFiles.length }) : t('compress_pdf.compress_file')}
                    </div>
                  )}
                </button>
              </div>
              ...
              {compressedFile && !batchMode && (
                <div className="mt-6 p-6 bg-green-50 border border-green-200 rounded-xl">
                  <div className="flex items-center mb-4">
                    ...
                    <h4 className="text-lg font-semibold text-green-800">{t('compress_pdf.compression_complete')}</h4>
                  </div>
                  <p className="text-green-700 mb-4">
                    {t('compress_pdf.success_message')}
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      ...
                      {t('compress_pdf.download_compressed')}
                    </button>
                    <button
                      onClick={resetForm}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-semibold mb-6 flex items-center">
                ...
                {t('compress_pdf.compression_settings')}
              </h3>
              
              {/* Quality Slider */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  {t('compress_pdf.quality')}: {quality}%
                </label>
                <input
                  type="range"
                  min="10"
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-1">
                  <span>{t('compress_pdf.quality_low')}</span>
                  <span>{t('compress_pdf.quality_high')}</span>
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  {t('compress_pdf.quality_desc')}
                </p>
              </div>
              ...
              <div className="mb-6">
                <label className="flex items-center">
                  ...
                  />
                  <span className="ml-2 text-sm text-gray-700">{t('compress_pdf.optimize_pdf')}</span>
                </label>
                <p className="text-xs text-gray-500 mt-1 ml-6">
                  {t('compress_pdf.optimize_desc')}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-semibold mb-6 flex items-center">
                ...
                {t('compress_pdf.sidebar_title')}
              </h3>
              <div className="space-y-4">
                {[
                  t('compress_pdf.feature_1'),
                  t('compress_pdf.feature_2'),
                  t('compress_pdf.feature_3'),
                  t('compress_pdf.feature_4'),
                  t('compress_pdf.feature_5'),
                  t('compress_pdf.feature_6')
                ].map((feature, index) => (
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <h3 className="text-xl font-semibold mb-6 flex items-center">
                ...
                {t('compress_pdf.perfect_for_title')}
              </h3>
              <div className="space-y-3">
                {[
                  t('compress_pdf.use_case_1'),
                  t('compress_pdf.use_case_2'),
                  t('compress_pdf.use_case_3'),
                  t('compress_pdf.use_case_4'),
                  t('compress_pdf.use_case_5'),
                  t('compress_pdf.use_case_6')
                ].map((useCase, index) => (
                  <div key={index} className="flex items-center">
        ...
        <div className="mt-16 bg-white rounded-2xl shadow-xl p-8 sm:p-12">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-8 text-center">
            {t('compress_pdf.why_compress_title')}
          </h2>
          
          <div className="prose prose-lg max-w-none">
            <p className="text-lg text-gray-700 mb-6 leading-relaxed">
              {t('compress_pdf.seo_intro')}
            </p>

            <h3 className="text-2xl font-semibold text-gray-900 mb-4 mt-8">{t('compress_pdf.benefits_title')}</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="bg-blue-50 p-6 rounded-lg">
                <h4 className="text-xl font-semibold text-blue-900 mb-3">{t('compress_pdf.benefit_size_title')}</h4>
                <p className="text-gray-700">
                  {t('compress_pdf.benefit_size_text')}
                </p>
              </div>
              
              <div className="bg-purple-50 p-6 rounded-lg">
                <h4 className="text-xl font-semibold text-purple-900 mb-3">{t('compress_pdf.benefit_speed_title')}</h4>
                <p className="text-gray-700">
                  {t('compress_pdf.benefit_speed_text')}
                </p>
              </div>
              
              <div className="bg-green-50 p-6 rounded-lg">
                <h4 className="text-xl font-semibold text-green-900 mb-3">{t('compress_pdf.benefit_bandwidth_title')}</h4>
                <p className="text-gray-700">
                  {t('compress_pdf.benefit_bandwidth_text')}
                </p>
              </div>
              
              <div className="bg-orange-50 p-6 rounded-lg">
                <h4 className="text-xl font-semibold text-orange-900 mb-3">{t('compress_pdf.benefit_seo_title')}</h4>
                <p className="text-gray-700">
                  {t('compress_pdf.benefit_seo_text')}
                </p>
              </div>
            </div>

            <h3 className="text-2xl font-semibold text-gray-900 mb-4 mt-8">{t('compress_pdf.use_cases_title')}</h3>
            
            <div className="space-y-4 mb-8">
              <div className="flex items-start">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-3 mr-4 flex-shrink-0"></div>
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">{t('compress_pdf.use_case_website_title')}</h4>
                  <p className="text-gray-700">{t('compress_pdf.use_case_website_text')}</p>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">{t('compress_pdf.use_case_email_title')}</h4>
                  <p className="text-gray-700">{t('compress_pdf.use_case_email_text')}</p>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">{t('compress_pdf.use_case_storage_title')}</h4>
                  <p className="text-gray-700">{t('compress_pdf.use_case_storage_text')}</p>
                </div>
              </div>
              ...
              <div className="flex items-start">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">{t('compress_pdf.use_case_sharing_title')}</h4>
                  <p className="text-gray-700">{t('compress_pdf.use_case_sharing_text')}</p>
                </div>
              </div>
            </div>

            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-8 rounded-xl text-center">
              <h3 className="text-2xl font-bold mb-4">{t('compress_pdf.ready_title')}</h3>
              <p className="text-lg mb-6 opacity-90">
                {t('compress_pdf.ready_text')}
              </p>
