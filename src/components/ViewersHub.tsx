      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
        ...
        {searchQuery && filteredCategories.length === 0 && (
          <div className="text-center py-8 sm:py-12 bg-white rounded-xl border-2 border-dashed border-gray-300 mx-4 sm:mx-0">
            ...
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2">No viewers found</h3>
            <p className="text-sm sm:text-base text-gray-600 mb-4 px-4">Try searching with different keywords like "PDF", "Image", or "RAW"</p>
            <button
              onClick={() => setSearchQuery('')}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                      <div className="flex items-start sm:items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3 mb-1">
                            <h3 className="text-base sm:text-lg font-semibold text-gray-900 truncate">{viewer.name}</h3>
                            <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-medium self-start sm:self-center whitespace-nowrap">
                              {viewer.category}
                            </span>
                          </div>
                          <p className="text-gray-600 text-xs sm:text-sm leading-relaxed">{viewer.description}</p>
                        </div>
                      </div>
                      <div className="flex-shrink-0 sm:ml-4">
                        {(viewer.path === '/viewers/jpg' || viewer.path === '/viewers/png' || viewer.path === '/viewers/webp' || viewer.path === '/viewers/gif' || viewer.path === '/viewers/svg' || viewer.path === '/viewers/tiff' || viewer.path === '/viewers/bmp' || viewer.path === '/viewers/ico' || viewer.path === '/viewers/heic' || viewer.path === '/viewers/avif' || viewer.path === '/viewers/pdf' || viewer.path === '/viewers/rtf' || viewer.path === '/viewers/txt' || viewer.path === '/viewers/md' || viewer.path === '/viewers/docx' || viewer.path === '/viewers/odt' || viewer.path === '/viewers/odp' || viewer.path === '/viewers/otp' || viewer.path === '/viewers/pot' || viewer.path === '/viewers/pps' || viewer.path === '/viewers/ppt' || viewer.path === '/viewers/sdd' || viewer.path === '/viewers/sx' || viewer.path === '/viewers/stl' || viewer.path === '/viewers/xlsx' || viewer.path === '/viewers/csv' || viewer.path === '/viewers/ods' || viewer.path === '/viewers/json' || viewer.path === '/viewers/xml' || viewer.path === '/viewers/html' || viewer.path === '/viewers/css' || viewer.path === '/viewers/js' || viewer.path === '/viewers/py' || viewer.path === '/viewers/nef' || viewer.path === '/viewers/cr2' || viewer.path === '/viewers/dcr' || viewer.path === '/viewers/x3f' || viewer.path === '/viewers/arw' || viewer.path === '/viewers/raf' || viewer.path === '/viewers/orf' || viewer.path === '/viewers/dng' || viewer.path === '/viewers/pef') ? (
                          <a
                            href={viewer.path}
                            className="inline-flex items-center justify-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm font-medium transition-colors w-full sm:w-auto min-h-[44px] touch-manipulation"
                          >
                            <Eye className="w-4 h-4" />
                            <span>Open Viewer</span>
                          </a>
                        ) : (
                          <div className="inline-flex items-center justify-center space-x-2 bg-gray-400 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm font-medium cursor-not-allowed w-full sm:w-auto min-h-[44px]">
                            <Eye className="w-4 h-4" />
        ...
        <div className="mt-12 sm:mt-16 bg-white rounded-2xl shadow-lg p-6 sm:p-8 mx-4 sm:mx-0">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 text-center mb-6 sm:mb-8">Why Choose Our Viewers?</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            <div className="text-center">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                <Eye className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
              </div>
              <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">Instant Preview</h3>
              <p className="text-sm sm:text-base text-gray-600">View files instantly without downloading or installing software</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm sm:text-base text-gray-600">Files are processed locally and never stored on our servers</p>
            </div>
            <div className="text-center sm:col-span-2 lg:col-span-1">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-purple-600" />
              </div>
              <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">Universal Support</h3>
              <p className="text-sm sm:text-base text-gray-600">Support for 50+ file formats across all categories</p>
            </div>
          </div>
        </div>
      </div>
