...
export const DCRViewer: React.FC = () => {
  ...
  const handleFilesSelected = (files: File[]) => {
    // Filter only DCR/KDC/K25 files
    const dcrFiles = files.filter(file => {
      const extension = file.name.split('.').pop()?.toLowerCase();
      return ['dcr', 'kdc', 'k25'].includes(extension || '');
    });
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                DCR Viewer & Converter
              </h1>
              <p className="text-lg text-gray-600 mt-2">
                Upload, view, and convert Kodak RAW (DCR/KDC/K25) image files
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">
            Upload Kodak RAW Files
          </h2>
          <FileUpload 
            onFilesSelected={handleFilesSelected}
            acceptedFormats={['dcr', 'kdc', 'k25']}
            maxFiles={20}
          />
        </div>
        ...
        {selectedFiles.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">
              Preview Kodak RAW Files ({selectedFiles.length})
            </h2>
              ...
              {selectedFiles.map((file, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    {(file.size / 1024 / 1024).toFixed(2)} MB • Kodak RAW
                  </div>
                  <div className="flex space-x-2">
                    <button
                      onClick={() => setViewerFile(file)}
                      className="flex-1 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium py-3 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2"
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Professional RAW format from Kodak's digital camera systems
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Uncompressed RAW data with maximum image quality preservation
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Advanced RAW processing capabilities for professional workflows
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-gray-300 mb-6">
              Professional DCR viewer and converter for all your Kodak RAW processing needs.
            </p>
            
            <div className="flex items-center justify-center space-x-2 text-sm text-gray-300">
              <span>© 2025 MorphyIMG. Built for Kodak professionals.</span>
            </div>
          </div>
        </div>
      </footer>
