...
export const ICOViewer: React.FC = () => {
  ...
  const features = t('viewers.ico.features', { returnObjects: true }) as Array<{ title: string; description: string }>;
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-base sm:text-lg lg:text-xl text-red-100 mb-4 sm:mb-5 px-4 max-w-2xl">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-4 sm:mb-6">
                <div className="bg-white/10 backdrop-blur-sm rounded-full px-3 sm:px-4 py-1.5 sm:py-2 border border-white/20">
                  <span className="text-sm sm:text-base text-white font-medium">
                    {t('viewers.ico.quick_stats.multiple_sizes')}
                  </span>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-full px-3 sm:px-4 py-1.5 sm:py-2 border border-white/20">
                  <span className="text-sm sm:text-base text-white font-medium">
                    {t('viewers.ico.quick_stats.windows_native')}
                  </span>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-full px-3 sm:px-4 py-1.5 sm:py-2 border border-white/20">
                  <span className="text-sm sm:text-base text-white font-medium">
                    {t('viewers.ico.quick_stats.fast_processing')}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          ...
          {selectedFiles.length > 0 && (
            <div className="mb-8 text-center">
              <button
                ...
                className="inline-flex items-center gap-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-bold py-4 px-8 sm:px-10 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-xl text-lg sm:text-xl"
              >
                <Eye className="w-6 h-6" />
                <span>{t('viewers.ico.buttons.view_files')}</span>
              </button>
            </div>
          )}
                ...
                <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-2xl p-6 border border-red-100">
                  ...
                  <ul className="space-y-3">
                    {advantages.map((advantage, idx) => (
                      <li key={idx} className="text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: `<span class="text-red-600 font-bold mr-2">•</span>${advantage}` }} />
                    ))}
                  </ul>
                </div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-6 border border-orange-100">
                  ...
                  <ul className="space-y-3">
                    {useCases.map((useCase, idx) => (
                      <li key={idx} className="text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: `<span class="text-orange-600 font-bold mr-2">•</span>${useCase}` }} />
                    ))}
                  </ul>
                </div>
              </div>
              ...
              <table className="min-w-full bg-white/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-gray-200">
                <thead>
                  <tr className="bg-gradient-to-r from-red-500 to-orange-600">
                    <th className="px-6 py-4 text-left text-sm font-bold text-white">{t('viewers.ico.specs_header_label')}</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-white">{t('viewers.ico.specs_header_value')}</th>
                  </tr>
                </thead>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 mb-8">
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-white/50 shadow-lg">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('viewers.ico.seo_viewing_title')}</h3>
              <p className="text-gray-600 leading-relaxed">{t('viewers.ico.seo_viewing_text')}</p>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-white/50 shadow-lg">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('viewers.ico.seo_conversion_title')}</h3>
              <p className="text-gray-600 leading-relaxed">{t('viewers.ico.seo_conversion_text')}</p>
            </div>
            
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-white/50 shadow-lg">
              <div className="w-12 h-12 bg-gradient-to-br from-amber-500 to-red-600 rounded-xl flex items-center justify-center mb-4">
                <Palette className="w-6 h-6 text-white" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{t('viewers.ico.seo_optimization_title')}</h3>
              <p className="text-gray-600 leading-relaxed">{t('viewers.ico.seo_optimization_text')}</p>
            </div>
          </div>
          ...
          <div className="text-center mb-8">
            <button
              onClick={() => window.location.href = '/viewers'}
              className="inline-flex items-center gap-2 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-bold py-4 px-8 sm:px-10 rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
              <ArrowLeft className="w-5 h-5" />
              <span>{t('viewers.ico.buttons.back', 'Back to All Viewers').replace(/^←\s*/, '')}</span>
            </button>
          </div>
        </div>
