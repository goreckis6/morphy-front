...
import { Image, Upload, Eye, ArrowLeft, Code, Palette, Zap, Info, Star, Sparkles, Layers, FileImage } from 'lucide-react';
...
import './translations/svgViewerTranslations'; // Register translations
...
export const SVGViewer: React.FC = () => {
  ...
  const getBasePath = () => {
    ...
    if (path.includes('/viewers/svg')) {
      return path.replace('/editor', '').replace(/\/viewers\/svg.*/, '/viewers/svg');
    }
    // Handle language prefixes
    const langMatch = path.match(/\/([a-z]{2})\/viewers\/svg/);
    if (langMatch) {
      const [, lang] = langMatch;
      return `/${lang}/viewers/svg`;
    }
    return '/viewers/svg'; // fallback
  };

  // Get translations with fallbacks to prevent map errors
  const featuresRaw = t('viewers.svg.features', { returnObjects: true });
  const advantagesRaw = t('viewers.svg.advantages', { returnObjects: true });
  const useCasesRaw = t('viewers.svg.use_cases', { returnObjects: true });
  const specsRaw = t('viewers.svg.specs', { returnObjects: true });
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    setSelectedFiles((prev: File[]) => [...prev, ...svgFiles]);
  };
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.svg.quick_stats.vector_graphics', '✓ Vector Graphics')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.svg.quick_stats.scalable', '📐 Scalable')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.svg.quick_stats.fast_processing', '⚡ Fast Processing')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10">
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-white/50">
            ...
            <FileUpload 
              ...
              showTotalSize={true}
              translationKeys={{
                dragDropText: t('viewers.svg.upload.drag_drop_text'),
                clickBrowseText: t('viewers.svg.upload.click_browse_text'),
                chooseFilesButton: t('viewers.svg.upload.choose_files_button'),
                maxFilesInfo: t('viewers.svg.upload.max_files_info')
              }}
            />
          </div>
          ...
          {selectedFiles.length > 0 && (
            <div className="flex justify-center mb-8">
              <button
                ...
                className="group relative px-8 py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 hover:from-indigo-700 hover:via-purple-700 hover:to-violet-700 text-white font-bold rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl flex items-center gap-3 text-lg overflow-hidden"
              >
                <div className="absolute inset-0 bg-white/20 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
                <Eye className="w-6 h-6 relative z-10" />
                <span className="relative z-10">{t('viewers.svg.buttons.view_files')}</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 border border-indigo-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.svg.advantages_title')}</h3>
                  </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-2xl p-6 border border-purple-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.svg.use_cases_title')}</h3>
                  </div>
                  <ul className="space-y-3">
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.svg.specs_header_label')}</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.svg.specs_header_value')}</th>
                  </tr>
                </thead>
          ...
          <div className="bg-gradient-to-br from-white via-indigo-50/50 to-purple-50/50 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-indigo-100">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
                {t('viewers.svg.seo_title')}
              </h2>
            </div>
            
            <div className="prose max-w-none text-gray-700">
              <p className="text-base sm:text-lg mb-6 leading-relaxed">
                {t('viewers.svg.seo_intro')}
              </p>
              ...
              <div className="space-y-6">
                <div className="bg-white/60 rounded-xl p-6 border border-indigo-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.svg.seo_viewing_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.svg.seo_viewing_text')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-indigo-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.svg.seo_conversion_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.svg.seo_conversion_text')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-indigo-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.svg.seo_optimization_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.svg.seo_optimization_text')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="text-center mb-12">
            <a
              href="/viewers"
              className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 hover:from-indigo-700 hover:via-purple-700 hover:to-violet-700 text-white font-bold rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl text-base sm:text-lg group"
            >
              <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
              <span>{t('viewers.svg.buttons.back').replace(/^←\s*/, '')}</span>
            </a>
          </div>
        </div>
