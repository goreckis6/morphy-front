...
import { Camera, Upload, Eye, ArrowLeft, AlertCircle, Info, Star, Sparkles, Layers, FileImage, Zap, Palette } from 'lucide-react';
...
export const DNGViewer: React.FC = () => {
  ...
  const featuresRaw = t('viewers.dng.features', { returnObjects: true });
  const advantagesRaw = t('viewers.dng.advantages', { returnObjects: true });
  const useCasesRaw = t('viewers.dng.use_cases', { returnObjects: true });
  const specsRaw = t('viewers.dng.specs', { returnObjects: true });
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    console.log('DNG files after filter:', dngFiles.map(f => ({ name: f.name, size: f.size })));
    
    if (dngFiles.length === 0) {
      console.warn('No valid DNG files found');
      return;
    }
  ...
  const handleViewDNG = async (file: File) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        loadingWindow.document.write(`
          ...
          <html>
          <head>
            <title>${file.name} - DNG Preview</title>
            <style>
              body {
                margin: 0;
                padding: 0;
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50 to-amber-50">
        ...
        <div className="relative overflow-hidden">
          {/* Animated Background */}
          <div className="absolute inset-0 bg-gradient-to-br from-orange-600 via-amber-600 to-red-700">
            <div
          ...
          <div className="absolute -bottom-8 left-1/2 w-72 h-72 bg-red-400 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob animation-delay-4000"></div>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.dng.quick_stats.raw_format', '✓ RAW Format')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.dng.quick_stats.high_quality', '⚡ High Quality')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.dng.quick_stats.fast_processing', '🎨 Fast Processing')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl p-6 border border-orange-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.dng.advantages_title')}</h3>
                  </div>
                ...
                {/* Use Cases */}
                <div className="bg-gradient-to-br from-amber-50 to-red-50 rounded-2xl p-6 border border-amber-100">
                  <div className="flex items-center gap-3 mb-4">
                    <Layers className="w-6 h-6 text-amber-600" />
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.dng.use_cases_title', 'Use Cases')}</h3>
                  </div>
                  <ul className="space-y-3">
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.dng.specs_header_label')}</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.dng.specs_header_value')}</th>
                  </tr>
                </thead>
          ...
          <div className="bg-gradient-to-br from-white via-orange-50/50 to-amber-50/50 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-orange-100">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
                {t('viewers.dng.seo_title', 'Why Choose Our DNG Viewer?')}
              </h2>
            </div>
            
            <div className="prose max-w-none text-gray-700">
              <p className="text-base sm:text-lg mb-6 leading-relaxed">
                {t('viewers.dng.seo_intro', 'Our DNG viewer provides a seamless experience for viewing and managing Digital Negative files directly in your browser.')}
              </p>
              ...
              <div className="space-y-6">
                <div className="bg-white/60 rounded-xl p-6 border border-orange-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.dng.seo_viewing_title', 'Professional RAW Viewing')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.dng.seo_viewing_text', 'View DNG files with high-quality rendering, zoom controls, and navigation tools.')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-orange-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.dng.seo_features_title', 'Advanced Features')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.dng.seo_features_text', 'Access powerful DNG viewing features including search, zoom, and metadata display.')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-orange-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.dng.seo_security_title', 'Secure & Private')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.dng.seo_security_text', 'Your DNG files are processed securely and never stored on our servers.')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="text-center mb-12">
            <a
              href="/viewers"
              className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-orange-600 via-amber-600 to-red-600 hover:from-orange-700 hover:via-amber-700 hover:to-red-700 text-white font-bold rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl text-base sm:text-lg group"
            >
              <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
              <span>{t('viewers.dng.buttons.back').replace(/^←\s*/, '')}</span>
            </a>
          </div>
        </div>
