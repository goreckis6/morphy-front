...
export const EPSViewer: React.FC = () => {
  ...
  const handleFilesSelected = (files: File[]) => {
    // Filter only EPS files
    const epsFiles = files.filter(file => {
      const extension = file.name.split('.').pop()?.toLowerCase();
      return ['eps', 'eps2', 'eps3', 'epsf'].includes(extension || '');
    });
    setSelectedFiles(epsFiles);
  };
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                EPS Viewer & Converter
              </h1>
              <p className="text-lg text-gray-600 mt-2">
                Upload, view, and convert Encapsulated PostScript (EPS) files
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          ...
          <FileUpload 
            ...
            acceptedFormats={['eps', 'eps2', 'eps3', 'epsf']}
            maxFiles={40}
          />
        </div>
        ...
        {selectedFiles.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">
              Preview EPS Files ({selectedFiles.length})
            </h2>
              ...
              {selectedFiles.map((file, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-xs text-gray-500 mb-3">
                    {(file.size / 1024).toFixed(1)} KB • Vector EPS
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Scalable PostScript graphics that maintain quality at any resolution
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              View and analyze the underlying PostScript code and commands
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Industry-standard format for professional printing and publishing
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-gray-300 mb-6">
              Professional EPS viewer and converter for all your vector graphics needs.
            </p>
            
            <div className="flex items-center justify-center space-x-2 text-sm text-gray-300">
              <span>© 2025 MorphyIMG. Built for EPS professionals.</span>
            </div>
          </div>
        </div>
      </footer>
