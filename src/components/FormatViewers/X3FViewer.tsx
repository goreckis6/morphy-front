...
import { Camera, Upload, ArrowLeft, CheckCircle, AlertCircle, Info, Image as ImageIcon, Maximize2, Zap, Star, Sparkles, Layers, FileImage } from 'lucide-react';
...
export const X3FViewer: React.FC = () => {
  ...
  const features = getFallbackArray<{ title: string; description: string }>('viewers.x3f.features');
  const advantages = getFallbackArray<string>('viewers.x3f.advantages');
  const compatibleCameras = getFallbackArray<string>('viewers.x3f.compatible_cameras');
  const specs = getFallbackArray<{ label: string; value: string }>('viewers.x3f.specs');
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50 to-cyan-50">
        ...
        <div className="relative overflow-hidden">
          {/* Animated Background */}
          <div className="absolute inset-0 bg-gradient-to-br from-sky-600 via-cyan-600 to-blue-700">
            <div 
          ...
          {/* Floating Orbs */}
          <div className="absolute top-20 left-10 w-72 h-72 bg-sky-400 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-blob"></div>
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.x3f.quick_stats.raw_format', '✓ RAW Format')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.x3f.quick_stats.high_quality', '🎨 High Quality')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.x3f.quick_stats.fast_processing', '⚡ Fast Processing')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10">
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-white/50">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-4 bg-gradient-to-br from-sky-500 to-cyan-600 rounded-2xl shadow-lg transform rotate-3 hover:rotate-0 transition-transform duration-300">
            ...
            <FileUpload 
              ...
              showTotalSize={true}
              translationKeys={{
                dragDropText: t('viewers.x3f.upload.drag_drop_text', 'Drag & Drop Your Files Here'),
                clickBrowseText: t('viewers.x3f.upload.click_browse_text', 'or click the button below to browse'),
                chooseFilesButton: t('viewers.x3f.upload.choose_files_button', 'Choose Files'),
                maxFilesInfo: t('viewers.x3f.upload.max_files_info', 'Max {maxFiles} files • Up to {maxSize} Total')
              }}
            />
            {validationError && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {features.map((feature, index) => {
              ...
              const iconNodes = [
                <ImageIcon className="w-8 h-8 text-white" key="image" />,
                <Maximize2 className="w-8 h-8 text-white" key="maximize" />,
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-white/50">
            <div className="flex items-center gap-4 mb-8">
              <div className="p-4 bg-gradient-to-br from-sky-500 to-cyan-600 rounded-2xl shadow-lg">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gradient-to-br from-sky-50 to-cyan-50 rounded-2xl p-6 border border-sky-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.x3f.advantages_title')}</h3>
                  </div>
                ...
                <div className="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-2xl p-6 border border-cyan-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.x3f.compatible_cameras_title')}</h3>
                  </div>
                  <ul className="space-y-3">
                    {compatibleCameras.map((item, idx) => {
                      const cleanItem = item.replace(/^•\s*/, '').trim();
                      return (
                        <li key={idx} className="flex items-start gap-3 text-gray-700">
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.x3f.specs_header_label')}</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.x3f.specs_header_value')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {specs.map((row, idx) => (
                    <tr key={row.label || idx} className={`hover:bg-sky-50/50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
          ...
          <div className="bg-gradient-to-br from-white via-sky-50/50 to-cyan-50/50 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-sky-100">
            <div className="flex items-center gap-4 mb-8">
              <div className="p-4 bg-gradient-to-br from-sky-500 to-cyan-600 rounded-2xl shadow-lg">
                <Sparkles className="w-8 h-8 text-white" />
              </div>
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
                {t('viewers.x3f.seo_title', 'Why Choose Our X3F Viewer?')}
              </h2>
            </div>
            
            <div className="prose max-w-none text-gray-700">
              <p className="text-base sm:text-lg mb-6 leading-relaxed">
                {t('viewers.x3f.seo_intro', 'Our X3F viewer provides a seamless experience for viewing and managing Sigma RAW files directly in your browser.')}
              </p>
              
              <div className="space-y-6">
                <div className="bg-white/60 rounded-xl p-6 border border-sky-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    <Camera className="w-5 h-5 text-sky-600" />
                    {t('viewers.x3f.seo_viewing_title', 'Professional RAW Viewing')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.x3f.seo_viewing_text', 'View X3F files with high-quality rendering, zoom controls, and navigation tools.')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-sky-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.x3f.seo_features_title', 'Advanced Features')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.x3f.seo_features_text', 'Access powerful X3F viewing features including metadata display, zoom, and rotation controls.')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-sky-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.x3f.seo_security_title', 'Secure & Private')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.x3f.seo_security_text', 'Your X3F files are processed securely and never stored on our servers.')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="text-center mb-12">
            <a
              href="/viewers"
              className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-sky-600 via-cyan-600 to-blue-600 hover:from-sky-700 hover:via-cyan-700 hover:to-blue-700 text-white font-bold rounded-2xl transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl text-base sm:text-lg group"
            >
              <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
              <span>{t('viewers.x3f.buttons.back', 'Back to All Viewers').replace(/^←\s*/, '')}</span>
            </a>
          </div>
        </div>
