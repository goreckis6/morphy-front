...
import './translations/webpViewerTranslations'; // Register translations
...
export const WebPViewer: React.FC = () => {
  ...
  // Get the base path for webp
  const getBasePath = () => {
    const path = location.pathname;
    if (path.includes('/viewers/webp')) {
      return path.replace('/editor', '').replace(/\/viewers\/webp.*/, '/viewers/webp');
    }
    // Handle language prefixes
    const langMatch = path.match(/\/([a-z]{2})\/viewers\/webp/);
    if (langMatch) {
      const [, lang] = langMatch;
      return `/${lang}/viewers/webp`;
    }
    return '/viewers/webp'; // fallback
  };
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    setSelectedFiles((prev: File[]) => [...prev, ...webpFiles]);
  };
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.webp.quick_stats.modern_format', '✓ Modern Format')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.webp.quick_stats.superior_compression', '⚡ Superior Compression')}</span>
                </div>
                <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md rounded-full border border-white/30 shadow-md">
                  <span className="text-white text-xs sm:text-sm font-semibold">{t('viewers.webp.quick_stats.high_quality', '🎨 High Quality')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 relative z-10">
          ...
          {selectedFiles.length > 0 && (
            <div className="flex justify-center mb-8">
              ...
              >
                ...
                <span className="relative z-10">{t('viewers.webp.buttons.view_files')}</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.webp.advantages_title')}</h3>
                  </div>
                ...
                <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-2xl p-6 border border-teal-100">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-gray-900">{t('viewers.webp.use_cases_title')}</h3>
                  </div>
                  <ul className="space-y-3">
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.webp.specs_header_label')}</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">{t('viewers.webp.specs_header_value')}</th>
                  </tr>
                </thead>
          ...
          <div className="bg-gradient-to-br from-white via-green-50/50 to-emerald-50/50 backdrop-blur-xl rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 mb-8 border border-green-100">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">
                {t('viewers.webp.seo_title')}
              </h2>
            </div>
            
            <div className="prose max-w-none text-gray-700">
              <p className="text-base sm:text-lg mb-6 leading-relaxed">
                {t('viewers.webp.seo_intro')}
              </p>
              ...
              <div className="space-y-6">
                <div className="bg-white/60 rounded-xl p-6 border border-green-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.webp.seo_viewing_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.webp.seo_viewing_text')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-green-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.webp.seo_conversion_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.webp.seo_conversion_text')}
                  </p>
                </div>
                ...
                <div className="bg-white/60 rounded-xl p-6 border border-green-100">
                  <h3 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
                    ...
                    {t('viewers.webp.seo_optimization_title')}
                  </h3>
                  <p className="text-gray-700 leading-relaxed">
                    {t('viewers.webp.seo_optimization_text')}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="text-center mb-12">
            ...
            >
              ...
              <span>{t('viewers.webp.buttons.back', 'Back to All Viewers').replace(/^←\s*/, '')}</span>
            </a>
          </div>
        </div>
