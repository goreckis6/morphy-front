...
export const CR2Viewer: React.FC = () => {
  ...
  const handleFilesSelected = (files: File[]) => {
    // Filter only CR2/CRW files
    const cr2Files = files.filter(file => {
      const extension = file.name.split('.').pop()?.toLowerCase();
      return ['cr2', 'crw'].includes(extension || '');
    });
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                CR2 Viewer & Converter
              </h1>
              <p className="text-lg text-gray-600 mt-2">
                Upload, view, and convert Canon RAW (CR2/CRW) image files
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">
            Upload Canon RAW Files
          </h2>
          <FileUpload 
            onFilesSelected={handleFilesSelected}
            acceptedFormats={['cr2', 'crw']}
            maxFiles={20}
          />
        </div>
        ...
        {selectedFiles.length > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">
              Preview Canon RAW Files ({selectedFiles.length})
            </h2>
              ...
              {selectedFiles.map((file, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    {(file.size / 1024 / 1024).toFixed(2)} MB • Canon RAW
                  </div>
                  <div className="flex space-x-2">
                    <button
                      onClick={() => setViewerFile(file)}
                      className="flex-1 bg-red-600 hover:bg-red-700 text-white text-sm font-medium py-3 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2"
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Canon's professional RAW format with maximum image quality and editing flexibility
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Support for Canon's advanced Dual Pixel RAW technology for enhanced post-processing
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <p className="text-gray-600">
              Maintain Canon's renowned color science and skin tone reproduction
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-gray-300 mb-6">
              Professional CR2 viewer and converter for all your Canon RAW processing needs.
            </p>
            
            <div className="flex items-center justify-center space-x-2 text-sm text-gray-300">
              <span>© 2025 MorphyIMG. Built for Canon professionals.</span>
            </div>
          </div>
        </div>
      </footer>
